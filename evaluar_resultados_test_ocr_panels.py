...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description='Compara resultado.txt con gt.txt (distancia Levenshtein)')
    parser.add_argument('--panels_path', default='./test_ocr_panels',
                        help='Directorio con gt.txt y resultado.txt')
    pargs = parser.parse_args()

    print('PROCESANDO testing_ocr ------------------------------')
    panels_gt = read_csv_file(os.path.join(pargs.panels_path, 'gt.txt'))
    print(panels_gt)

    panels = read_csv_file(os.path.join(pargs.panels_path, 'resultado.txt'))
    print(panels)
